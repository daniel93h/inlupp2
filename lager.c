...
bool shelf_exists(tree_t *tree, elem_t shelf)
{
  elem_t *elements = tree_elements(tree);

  for(int i = 0 ; i < tree_size(tree)-1 ; i++)
    {
      elem_t item = elements[i];
      list_t *shelves = ((item_t *) item.p)->shelves;
      
      if (list_has_shelf(shelves, shelf))
        {
